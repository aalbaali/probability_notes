\documentclass{amro-notes}
\usepackage{amro-common}

% Generates the index
\usepackage{makeidx}
\makeindex

\setcounter{secnumdepth}{5}
\setcounter{tocdepth}{5}

\title{Probability Notes}
\author{Amro Al Baali}

\newcommand{\daterange}{Compiled: \today}
\date{\daterange}

\renewcommand{\symProb}{P}
\newcommand{\stringRVs}[2]{\rv{#1}_{1},\ldots,\rv{#1}_{#2}}

\renewcommand{\herm}{\ensuremath{\ast}}

\renewcommand{\symFourier}{\texttt{F}}

\linespread{1.125}
\allowdisplaybreaks
\setcounter{tocdepth}{2}

\graphicspath{{../}{../figs/}{figs/}}

% Add bibliography
%\bibliography{references.bib}
\bibliography{../references.bib}

\begin{document}
\maketitle
\tableofcontents
\pagenumbering{Roman} 


\chapter*{Preface}
\input{sections/1.0_Preface.tex}

\clearpage
\chapter*{Notations}
\addcontentsline{toc}{chapter}{Notations}
\input{sections/notations.tex}

\clearpage
\pagenumbering{arabic} 
\part{Fundamentals}
\label{part:fundamentals}

\chapter{Single Variable Probability}
\input{sections/1.1_Ch1.tex}

\clearpage
\chapter{Multivariate Probability}
\input{sections/2_Multivariate.tex}

\clearpage
\chapter{Distributions}
\input{sections/ch_distributions.tex}

\clearpage
\chapter{Graphs in probability}
\input{sections/ch_graphs.tex}

\clearpage
\part{Statistical Inference}

\chapter{Prediction}
\input{sections/ch_prediction.tex}

\chapter{Parameter Estimation}
\input{sections/ch_estimation.tex}

\chapter{Hypothesis Testing}
\input{sections/ch_hypothesisTesting.tex}

\part{Random Processes and Sequences}
\chapter{Definitions and Terminology}
\input{sections/ch_rp_def.tex}

\chapter{Stochastic Convergence}
\input{sections/ch_rp_convergence.tex}

\chapter{Stationary Processes}
\input{sections/ch_rp_stationay.tex}

\chapter{Point and Counting Processes}
\input{sections/ch_rp_counting.tex}

\chapter{Markov processes}
\input{sections/ch_markov.tex}

\chapter{Ergodicity}
\input{sections/ch_ergodicity.tex}

\begin{appendices}
  \input{sections/appendices.tex}
\end{appendices}


% Add bib
\clearpage
\addcontentsline{toc}{chapter}{Bibliography}
\printbibliography


% Add indices
\addcontentsline{toc}{chapter}{Indices}
\printindex

\end{document}
